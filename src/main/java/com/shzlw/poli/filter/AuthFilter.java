...
public class AuthFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		if (authBySessionKey(httpRequest, path) || authByApiKey(httpRequest, path) || authByShareKey(httpRequest, path)
				|| authForDrillThrough(httpRequest, path)) {
			chain.doFilter(request, response);
			return;
		}
	...
	private static boolean validateByShareKey(String requestMethod, String path, SharedLinkInfo linkInfo,
			String shareKey) {
		if (linkInfo == null) {
			return false;
		}
		...
		if (Constants.HTTP_METHOD_GET.equals(requestMethod)) {
			if (path.equals("/ws/reports/" + reportId) || path.equals("/ws/components/report/" + reportId)
	...
	private boolean authForDrillThrough(HttpServletRequest httpRequest, String path) {
		if (Constants.HTTP_METHOD_GET.equals(httpRequest.getMethod())) {
			if (path.matches("/ws/reports/[\\d]+")) {
				String repId = path.split("/ws/reports/")[1];
				Long reportId = null;
				if (repId == null || repId.length() == 0) {
					return false;
				} else {
					reportId = Long.parseLong(repId);
				}

				SharedLinkInfo linkInfo = sharedReportService.getSharedLinkInfoByReportId(reportId);
				if (linkInfo == null) {
					return false;
				}
				...
				httpRequest.setAttribute(Constants.HTTP_REQUEST_ATTR_USER, user);
				return validateByShareKey(httpRequest.getMethod(), path, linkInfo, null);
			} else {
				return true;
			}
		} else if (Constants.HTTP_METHOD_POST.equals(httpRequest.getMethod())) {
			return true;
		}
		return false;
	}
