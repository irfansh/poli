...
class ComponentEditPanel extends React.Component {
	...
	handleComponentOptionChange = (name, value) => {
		this.setState({
			[name]: value
		});
	}
	...
	save = () => {
		...
		if (type === Constants.CHART) {
			...
			if (subType === Constants.SLICER) {
				component.data["slicerType"] = this.state.slicerType;
			}
		}
	...
	renderChartConfigPanel = () => {
		...
		if (subType === Constants.TABLE || subType === Constants.RATINGTABLE) {
			const {
				...
				showPagination = true,
				fixedHeader = false,
				isRatingTable = subType === Constants.RATINGTABLE,
			} = data;
			chartConfigPanel = (
				<div className="form-panel">
					...
					{isRatingTable && (
						<div>
							...
							<label>{t('Default Rating value')}</label>
							<Select
								name={'defaultRatingStep'}
								value={defaultRatingStep}
								onChange={this.handleComponentDataChange}
							...
							<Select
								name={'starColor'}
								onChange={this.handleComponentDataChange}
								options={Constants.STAR_COLOR_LIST}
							/>
						</div>
					)}
							...
							<div className="form-panel" style={{ paddingTop: '10px' }}>
								...
								{type === Constants.FILTER && (
									<div>
										...
										{subType === Constants.SLICER && (
											<div>
												<label>{t('Slicer Type')} <span style={{ color: '#8993A4', fontSize: '15px' }}>Text</span></label>
												<Select
													name={'slicerType'}
													value={this.state.slicerType}
													className="form-input"
													options={Constants.SLICER_TYPES}
													onChange={this.handleComponentOptionChange}
												/>
											</div>
										)}
									</div>
