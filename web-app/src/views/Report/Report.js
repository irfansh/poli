...
class Report extends Component {
	...
	import = () => {
		...
		axios.post('/ws/reports/import', formData)
			.then(res => {
		...
		return (
			<React.Fragment>
				<div className="report-sidebar">
					...
					<div style={{ margin: '8px 5px 5px 5px' }}>
						<button className="button full-width" onClick={() => this.setState({ showImportPanel: true })}>
							<FontAwesomeIcon icon="file-import" fixedWidth /> {t('Import')}
						</button>
					</div>
					<div style={{ margin: '8px 5px 5px 5px' }}>
						<SearchInput
				...
				<Modal
					show={this.state.showImportPanel}
					onClose={() => this.setState({ showImportPanel: false })}
					modalClass={'small-modal-panel'}
					title={t('New')} >
					<div className="form-panel">
						<label>{t('Name')}</label>
						...
						<button className="button button-green" onClick={this.import}>
							<FontAwesomeIcon icon="save" fixedWidth /> {t('Save')}
						</button>
					</div>
				</Modal>
