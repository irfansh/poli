...
class ReportEditView extends React.Component {
	...
	exportCurrentReport = () => {
		...
		axios.get(`/ws/reports/export/${reportId}`)
			.then(res => {
				const repData = JSON.stringify(res.data);
				const fileName = res.headers["content-disposition"].split(";")[1].split("=")[1];
				const blob = new Blob([repData]);
			...
			<React.Fragment>
				...
				>
					<div className="form-panel">
						...
						<button className="button square-button button-transparent ml-4" onClick={this.exportCurrentReport}>
