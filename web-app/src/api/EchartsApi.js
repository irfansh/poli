...
export const getChartOption = (type, data, config, title) => {
	...
	} else if (type === Constants.GAUGE) {
		chartOption = getGaugeOption(data, config);
	}
	return chartOption;
}
...
const getBarOptionTemplate = (colorPlatte = 'default', legendData, axisData, series, config = {}) => {
	...
	if (typeof markline !== 'undefined' && markline !== '') {
		//add markline
		const barAxis = isHorizontal ? { xAxis: markline } : { yAxis: markline };
		const markLine = {
			lineStyle: {
				color: marklineColor,
				type: 'dashed',
...
const getGaugeOptionTemplate = (colorPlatte = 'default', legendData, seriesData, config = {}) => {

}
/**
* Gauge Chart
 */
const getGaugeOption = (data, config) => {
	const {
		gaugeValue,
		maxValue = 100,
		gaugeColor = [
	...
	for (let i = 0; i < data.length; i++) {
		const row = data[i];
		gaugeData.push({ value: row[gaugeValue] });
	}
	if (gaugeData.length === 0) {
		gaugeData.push({ value: 0 });
	}
	return {
		series: [{
			type: 'gauge',
