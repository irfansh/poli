...
import SelectSlicer from './filters/SelectSlicer';
...
class GridItem extends React.PureComponent {
	...
	renderComponentContent = () => {
		...
		if (type === Constants.CHART) {
			if (subType === Constants.TABLE) {
			...
			} else if (subType === Constants.RATINGTABLE) {
				const {
					ratingColumn,
				...
				componentItem = (
					<RatingTable
						...
						defaultRatingStep={defaultRatingStep}
						starColor={starColor}
					/>
				);
			...
			if (subType === Constants.SLICER) {
				if (typeof data.slicerType !== "undefined" && data.slicerType === Constants.DROPDOWN) {
					componentItem = (
